...
class WiFiConfigController {
	...
	protected:
		...
		/**
		 * @var	CallBackIntArgFn|NULL	_ewstack_callback_fn
		 */
	...
	public:
		...
		/**
		 * register wifi config route handler
		 *
		...
		void handle( EwWebResourceProvider* _web_resource ){
			...
			this->web_resource->register_route( EW_SERVER_WIFI_CONFIG_ROUTE, [&]() { this->handleWiFiConfigRoute(); }, AUTH_MIDDLEWARE );
		}

		/**
		 * set wifi config callback. called when wifi config modified by client.
		 *
      ...
      concat_tr_input_html_tags( _page, PSTR("Access Name:"), PSTR("ap_ssid"), this->web_resource->wifi_configs.ap_ssid, WIFI_CONFIGS_BUF_SIZE-1 );
      concat_tr_input_html_tags( _page, PSTR("Access Password:"), PSTR("ap_pswd"), this->web_resource->wifi_configs.ap_password, WIFI_CONFIGS_BUF_SIZE-1 );
    ...
    void handleWiFiConfigRoute( void ) {
      ...
      if ( this->web_resource->EwServer->hasArg("sta_ssid") && this->web_resource->EwServer->hasArg("sta_pswd") ) {
        ...
        String _sta_pswd = this->web_resource->EwServer->arg("sta_pswd");
        String _sta_lip = this->web_resource->EwServer->arg("sta_lip");
        String _sta_gip = this->web_resource->EwServer->arg("sta_gip");
        String _sta_sip = this->web_resource->EwServer->arg("sta_sip");

        String _ap_ssid = this->web_resource->EwServer->arg("ap_ssid");
        String _ap_pswd = this->web_resource->EwServer->arg("ap_pswd");
        String _ap_lip = this->web_resource->EwServer->arg("ap_lip");
        String _ap_gip = this->web_resource->EwServer->arg("ap_gip");
        String _ap_sip = this->web_resource->EwServer->arg("ap_sip");
          ...
          _sta_lip.toCharArray( _ip_address, _sta_lip.length()+1 ); __str_ip_to_int(_ip_address, this->web_resource->wifi_configs.sta_local_ip, 20);
          _sta_gip.toCharArray( _ip_address, _sta_gip.length()+1 ); __str_ip_to_int(_ip_address, this->web_resource->wifi_configs.sta_gateway, 20);
          _sta_sip.toCharArray( _ip_address, _sta_sip.length()+1 ); __str_ip_to_int(_ip_address, this->web_resource->wifi_configs.sta_subnet, 20);

          _ap_ssid.toCharArray( this->web_resource->wifi_configs.ap_ssid, _ap_ssid.length()+1 );
          _ap_pswd.toCharArray( this->web_resource->wifi_configs.ap_password, _ap_pswd.length()+1 );
          _ap_lip.toCharArray( _ip_address, _ap_lip.length()+1 ); __str_ip_to_int(_ip_address, this->web_resource->wifi_configs.ap_local_ip, 20);
