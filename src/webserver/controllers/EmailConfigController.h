...
class EmailConfigController : public Controller {
	...
	public:
		 ...
		 */
		void build_email_config_html( char* _page, bool _is_error=false, bool _enable_flash=false, int _max_size=EW_HTML_MAX_SIZE ){
      ...
      concat_tr_input_html_tags( _page, PSTR("Mail Password:"), PSTR("ml_psw"), this->email_configs.mail_password, DEFAULT_MAIL_PASSWORD_MAX_SIZE-1 );
      ...
      if( _enable_flash )
      concat_flash_message_div( _page, _is_error ? PSTR("Invalid length error"): PSTR("Config saved Successfully"), _is_error ? ALERT_DANGER:ALERT_SUCCESS );
    ...
    void handleEmailConfigRoute( void ) {
      ...
      if ( this->web_resource->server->hasArg("ml_dmn") && this->web_resource->server->hasArg("ml_srvr") ) {
        ...
        String _mail_domain = this->web_resource->server->arg("ml_dmn");
          ...
          // this->set_email_config_table( &this->email_configs );
          _is_error = false;
        }
        _is_posted = true;
      }
      ...
      this->build_email_config_html( _page, _is_error, _is_posted );

      if( _is_posted && !_is_error ){

				__email_service.handleEmails();
      }
