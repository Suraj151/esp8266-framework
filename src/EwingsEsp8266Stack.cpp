...
device_wifi_status_t _wifi_status = {
  false, false, 0, {0}
};
...
void EwingsEsp8266Stack::initialize(){
  ...
  this->run_while_factory_reset( [&]() { this->clear_default_tables(); this->wifi->disconnect(true); } );
  ...
  this->beginEspNow(this->wifi,this->EwingsDefaultDatabase());
  this->setInterval( [&]() { this->handlePeers(); }, 5000 );
  #endif
}
 ...
 */
void EwingsEsp8266Stack::handleInternetConnectivity(){
  ...
  if( !this->wifi->localIP().isSet() || !this->wifi->isConnected() ){
    ...
    memset( &_wifi_status, 0, sizeof(device_wifi_status_t) );
    _wifi_status.last_internet_millis = millis();
  }else{

    bool ping_ret = this->ping();
    bool ping_resp = this->isHostRespondingToPing();

    if( ping_ret && ping_resp ){

      _wifi_status.last_internet_millis = millis();
    }
    _wifi_status.wifi_connected = true;
    _wifi_status.internet_available = ping_ret && ping_resp;

    if( !_wifi_status.internet_available && ((millis()-_wifi_status.last_internet_millis)/INTERNET_CONNECTIVITY_CHECK_DURATION) >= 6 ){

      memcpy( _wifi_status.ignore_bssid, this->wifi->BSSID(), 6 );
      ...
      this->setTimeout( [&]() {
        ...
        #endif
        memset( _wifi_status.ignore_bssid, 0, 6 );
        if( !this->wifi->localIP().isSet() || !this->wifi->isConnected() ){
          wifi_config_table _wifi_credentials = this->get_wifi_config_table();
          this->configure_wifi_station( &_wifi_credentials );
        }
  ...
  #ifdef EW_SERIAL_LOG
  Log( _wifi_status.internet_available ); Log( F(" : ") );
  Logln( (millis()-_wifi_status.last_internet_millis) );
  #endif
}
 ...
 * @return  uint32_t
 */
uint32_t EwingsEsp8266Stack::getStationSubnetIP(void){
 ...
 * @return  uint32_t
 */
uint32_t EwingsEsp8266Stack::getStationBroadcastIP(void){
 ...
 * @return  bool
 */
bool EwingsEsp8266Stack::configure_wifi_station( wifi_config_table* _wifi_credentials, uint8_t* mac ){
 ...
 *
 */
void EwingsEsp8266Stack::reconfigure_wifi_access_point( void ){
  ...
  if( !this->wifi->localIP().isSet() || !this->wifi->isConnected() ){
    ...
    if(
      ...
      this->set_wifi_config_table(&_wifi_credentials);
      _ap_change = true;
    }
  }else{
  ...
  if( _ap_change ){
    ...
    this->setTimeout( [&](){
      wifi_config_table __wifi_credentials = this->get_wifi_config_table();
 ...
 * @return  bool
 */
bool EwingsEsp8266Stack::scan_within_station_async( char* ssid, uint8_t* bssid, int _scanCount ){
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if( __are_arrays_equal( ssid, _ssid_buff, _ssid.length()+1 ) ){
      ...
      if( !_found ){
        ...
        if( !__are_arrays_equal( (char*)_wifi_status.ignore_bssid, (char*)this->wifi->BSSID(i), 6 ) ){
 ...
 * @return  bool
 */
bool EwingsEsp8266Stack::scan_within_station( char* ssid, uint8_t* bssid ){
 ...
 */
void EwingsEsp8266Stack::scan_aps_and_configure_wifi_station_async( int _scanCount ){
 ...
 *
 */
void EwingsEsp8266Stack::scan_aps_and_configure_wifi_station( ){
